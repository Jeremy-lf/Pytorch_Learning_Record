...
def conv(name, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
    return op_name(name, nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias))
...
def bn_relu_conv(name, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1):
    return nn.Sequential(
        op_name(name + '/bn', nn.BatchNorm2d(in_channels)),
        op_name(name + '/relu', nn.ReLU(inplace=True)),
        op_name(name, nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, False)),
    )

def bn_relu(name, in_channels):
    return nn.Sequential(
        op_name(name + '/bn', nn.BatchNorm2d(in_channels)),
        op_name(name + '/relu', nn.ReLU(inplace=True)),
    )
...
def convert_weight_from_pytorch_to_caffe(torch_net, caffe_net):
    for name, m in torch_net.named_modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
            print('convert conv:', name, m.op_name, m)
